    ...
    public class SalaryCalculator
    {
         ...
         public decimal GetAnnualSalary(decimal hourlyWage) // calculates annual salary 
        {
            if (hourlyWage > 0)
                ...
                throw new InvalidOperationException("Hourly wage must not be less than zero.");// throw exception if negative wages 
        }
        ...
        public EmpData EmpAccount(decimal weeklySalary, int numDependents) // calculate tax data 
        {
            if (weeklySalary > 0) {
                decimal dd = 0;
